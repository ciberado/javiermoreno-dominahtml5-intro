...
function GestionSuperHeroesServiceSQL() {
	...
	this._db.transaction(function(tx) {
		...
		for (var idx=0; idx < SuperHeroe.SUPER_HEROES_EJEMPLO.length; idx++) {
			self._insertSuperHeroeAuto(tx, SuperHeroe.SUPER_HEROES_EJEMPLO[idx]);
		}
		$(self).trigger(GestionSuperHeroesService.ON_READY_EVENT);
	});
}

GestionSuperHeroesServiceSQL.prototype.registrar = function(superHeroe, fnSuccess) {
	var self = this;
	this._db.transaction(function(tx) {
	    self._insertSuperHeroe(tx, superHeroe, fnSuccess);
	});
}
...
GestionSuperHeroesServiceSQL.prototype._insertSuperHeroeAuto = function(tx, superHeroe, fn) {
...
GestionSuperHeroesServiceSQL.prototype._insertSuperHeroe = function(tx, superHeroe, fn) {
	var sql = 'INSERT INTO superHeroes '
	        + '    (nombre, identidadSecreta, grupo, urlImagen) '
			+ 'VALUES (?, ?, ?, ?) ';
	tx.executeSql(sql, [superHeroe.nombre,
						superHeroe.identidadSecreta,
						superHeroe.grupo, superHeroe.urlImagen], 
		fn, GestionSuperHeroesServiceSQL.prototype._defaultDatabaseError);
